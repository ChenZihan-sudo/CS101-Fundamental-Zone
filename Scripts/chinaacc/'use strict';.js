...
define(['jquery', 'h5/videoModel', 'h5/videoCtrl', 'h5/h5StatApi', 'h5/h5EventApi', 'h5/md5'],
    function ($, videoModel, videoCtrl, stats, vEvents, md5) {
        var h5timing = {
            _videoRefID: '', _studyID: '', _userID: '', _courseID: '', _duration: 0, _playSpeed: 1.0, _playTime: 0, _getSysTimeUrl: '', _openTime: null, _time: null, _postTimeKey: null, _timingState: null, _timePointCache: 0, _item: { pointStart: null, pointEnd: null, speed: 1.0, timeStart: null, timeEnd: null },
            _items: [], _traceInfo: { refType: 'video', source: 'pch5hls', rangeStart: '0', rangeEnd: '', items: null },
            _postDataUrl: '', _basePath: '', _st: 1000 * 60 * 3, setPostTimeOut: function (callback, delay) {
            ...
            initPlaySpeed: function () { this._playSpeed = videoModel.getPlayStatus().playSpeed },
            initDuration: function () {
                this._duration = stats.getDuration();
                if (!this._duration) {
                    var _this = this;
                    setTimeout(function () { _this.initDuration.call(_this) },
                        100)
            ...
            getPostInterval: function () { return videoModel.getPlayerOpts().postInterval },
            getSysTime: function (url, callback) {
                var _this = this;
                $.ajax({
            ...
            start: function () { if (videoModel.getPlayerOpts().isSaveTime !== 1) { return } this.init() },
            init: function () {
            ...
            _initEvents: function () {
                var _th = this;
                if (stats.getMediaStatus() === 'play') { _th.itemStart() } vEvents.sub('media.playing', function () {
                    window.setTimeout(function () { _th.itemStart() },
                        1000)
                });
            ...
            saveData: function (isAsync, isTimeCheck, isEnded) {
                if (isEnded == undefined) { isEnded = 0 }
                var _this = this;
                ...
                var innerTime = 0;
                for (var j = 0, len = traceInfo.items.length; j < len; j++) {
                    if (traceInfo.items[j].pointStart != null) { innerTime += traceInfo.items[j].pointEnd - traceInfo.items[j].pointStart }
                }
                if (isTimeCheck && (!_this.getTime() || (_this.getTime() - _this._openTime) < 8 * 1000)) { return false } if (innerTime > (60 * 5)) { innerTime = 300 }
                var videoID = _this._videoRefID;
                this.getSysTime(sysTimeUrl, function () {
                    var hs = h5timing._openTime;
                    var postTimeKey = h5timing._postTimeKey;
                    var keyStr = videoID.toString() + playTime.toString() +
                        innerTime.toString() + postTimeKey + hs;
                    var keyMd5Str = hex_md5(keyStr);
                    $.ajax({
                        url: basePath + postDataUrl, dataType: 'json', async: isAsync, type: 'post',
                        data: {
                            videoID: videoID,
                            studyID: _this._studyID,
